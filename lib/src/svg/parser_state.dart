...
class SvgParserState {
  ...
  SvgParserState(
    ...
    if (events.any((XmlEvent element) =>
        element is XmlStartElementEvent && element.name == 'defs')) {
  ...
  static Iterable<XmlEvent> reorderDefs(Iterable<XmlEvent> events) {
    ...
    if (events.first is XmlStartElementEvent &&
        (events.first as XmlStartElementEvent).name == 'svg') {
      return <XmlEvent>[
        events.first,
        ...reorderDefs(events.skip(1).take(events.length - 2)),
        events.last
      ];
    }
    ...
    for (int i = 0; i < events.length; i++) {
      ...
      if (event is XmlStartElementEvent) {
        ...
        depth--;
        if (depth == 0) {
          final Iterable<XmlEvent> children =
              events.skip(childStart).take(i - childStart + 1);
          if (event.name == 'defs') {
            ...
            if (!containsDefs) {
              ...
              result.addAll(<XmlEvent>[
                children.first,
                ...reorderDefs(children.skip(1).take(children.length - 2)),
                children.last
              ]);
            }
          }
